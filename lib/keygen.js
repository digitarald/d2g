...
exports.createKeypair = function(filename, cb) {
  ...
  var keygenTemplate = 'keytool -genkey -v -keystore <%-keystore%> -alias <%-alias%> -keyalg RSA -sigalg SHA384withRSA -keysize 1024 -validity 10000 -dname "CN=<%-commonName%>, OU=<%-organizationUnit%>, O=<%-organization%>, L=<%-city%>, S=<%-state%>, C=<%-countryCode%>" -storepass <%-store_password%> -keypass <%-alias_password%>';
...
exports.signAppPackage = function(inputFile, outputFile, cb, keygenParams) {
  // We need spawn, instead of exec, because we want to write the password in from stdin.
